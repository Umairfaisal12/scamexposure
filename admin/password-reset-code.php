...
if (isset($_POST['password_reset_link'])) {
    try {
        ...
        $check_email_query = "SELECT username, email FROM admin WHERE email = :email AND status = 1 LIMIT 1";
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $update_token_query = "UPDATE admin SET reset_token_hash = :token, reset_token_expires_at = :expiry WHERE email = :email";
            ...
            if ($update_stmt->execute()) {
                if (send_password_reset_link($username, $email, $token)) {
...
function send_password_reset_link($username, $email, $token) {
    ...
    try {
        ...
        // Create reset link
        $resetLink = "http://localhost/scmex/admin/reset-password.php?token=" . urlencode($token) . "&email=" . urlencode($email);
        ...
        $mail->Body = "
            <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 5px;'>
                ...
                <div style='margin-bottom: 30px;'>
                    <h3 style='color: #444;'>Hello {$username},</h3>
